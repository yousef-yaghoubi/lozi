...
type BlogCartFull = BlogCart & {
  type: "blog";
};
function Cart({ content }: { content: ProductCartFull | BlogCartFull }) {
  const [isHovered, setIsHovered] = useState(false);
  const width = useWindowWidth();

  return (
    <div
      className="flex flex-col w-full max-w-[165px] md:max-w-[432px] h-48 md:h-[496px] bg-white border md:border-2 border-primary rounded-[10px] md:rounded-2.5xl px-1 pb-2 pt-2 md:px-4 md:pt-6 md:pb-4 relative"
    ...
    >
      <>
        ...
        <h5 className="m-caption-bold md:w-h6 m-1 md:m-2 text-black-400">
          {TruncateString(content.title, 20)}
        </h5>
        {content.type == "product" ? (
          <>
            ...
            <div className="flex items-center gap-1 md:gap-2 absolute bottom-2 md:bottom-4 left-[13px] md:left-8">
              <span className="m-caption-bold md:w-h6">
                {numberToPersian(content.price.toLocaleString())}
              </span>
          ...
          <p className="m-caption-xs md:w-text-sm text-justify text-black-300">
            {TruncateString(content.desc, 120)}
          </p>
        )}
      </>
      ...
      <div
        className={cn(
          "min-w-[45%] w-fit h-8 md:h-[76px] bg-white z-10",
          content.type == "product"
      ...
      >
        <Button
          ...
          className="w-full"
        >
          <span>{content.type == "product" ? "خرید" : "مشاهده مقاله"}</span>
