...
const ButtonIcon: React.FC<ButtonProps> = ({
  ...
  function GetClassName(btn: "text" | "stroke" | "fill") {
    const baseClass = `flex items-center justify-around duration-300 transition-all ${
      size == "superSmall"
        ? " h-6 w-6 rounded-sm m-caption-sm-bold "
        : size == "small"
        ? " h-10 w-10 rounded-md w-caption-md-bold "
        : size == "medium"
        ? " h-14 w-14 rounded-lg w-text-sm-bold "
        : " h-16 w-16 rounded-xl w-text-md-bold "
    }`;
    const textClass =
